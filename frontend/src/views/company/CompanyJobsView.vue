...
const companyName = computed(() => getSession()?.user?.major?.trim() || getSession()?.user?.fullName?.trim() || '')
const form = reactive(emptyJob())

function emptyJob() {
  return {
    title: '', company: companyName.value, city: '', jobType: '实习',
...
async function loadJobs() {
  ...
  try { jobs.value = await api('/jobs/mine') } catch (e) { message.value = e.message || '岗位加载失败' } finally { loading.value = false }
}
...
function editJob(job) {
  ...
  Object.assign(form, {
    title: job.title, company: companyName.value || job.company, city: job.city,
    ...
    <div class="adm-grid-2">
      <form class="adm-form" @submit.prevent="saveJob">
        ...
        <div class="adm-form-row">
          ...
          <div class="adm-field"><label>类型</label><select v-model="form.jobType"><option>实习</option><option>校招</option></select></div>
        </div>
        <div class="adm-form-row">
        ...
        <div class="adm-form-row">
          ...
          <div class="adm-field"><label>状态</label><select v-model="form.status"><option value="OPEN">开放</option><option value="CLOSED">关闭</option></select></div>
        </div>
      ...
      <div class="adm-list-panel">
        <div class="adm-list-head"><h3>岗位列表</h3><span>{{ jobs.length }} 条</span></div>
        <div class="adm-list">
